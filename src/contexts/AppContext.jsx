...
export function AppProvider({ children }) {
  ...
  const registrableRoles = () => {
    if (currentUser?.role === 'ceo') return ['technician','team_leader','hod','accountant'];
    if (currentUser?.role === 'hod') return ['team_leader', 'technician'];
    if (currentUser?.role === 'team_leader') return ['technician'];
    return [];
  };
  ...
  const applyBonus = (userId, month, bonusAmount, bonusReason) => {
    setSalaries(p => p.map(s =>
      s.user_id===userId&&s.month===month
        ...
        : s));
    pushNotif(16,'salary',`CEO added bonus RWF ${bonusAmount.toLocaleString()} to ${users.find(u=>u.id===userId)?.name} for ${month}.`,'/salary');
    pushNotif(userId,'salary',`CEO added a performance bonus of RWF ${bonusAmount.toLocaleString()} to your salary for ${month}.`,'/salary');
  };
  ...
  const registerSupervisor = (form) => {
    const newUser = registerUser({ ...form, role: 'hod' });
    if (form.dept) {
