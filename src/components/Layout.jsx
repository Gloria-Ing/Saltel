...
const NAV = [
  { path:'/',              icon:'bi bi-grid-1x2-fill',       label:'Dashboard',           roles:['ceo','hod','team_leader','technician','accountant','daf'] },
  { path:'/daf-finance',  icon:'bi bi-bar-chart-line-fill', label:'Finance & Admin',     roles:['daf'] },
  { path:'/working-plan', icon:'bi bi-calendar3',           label:'Working Plan',        roles:['ceo','hod','team_leader','technician'] },
  { path:'/tasks',         icon:'bi bi-check2-square',       label:'Tasks',               roles:['ceo','hod','team_leader','technician'] },
  { path:'/attendance',    icon:'bi bi-geo-alt-fill',        label:'Attendance',          roles:['ceo','hod','team_leader','technician'] },
  { path:'/reports',       icon:'bi bi-file-earmark-text-fill', label:'Reports',         roles:['ceo','hod','team_leader','technician','accountant'] },
  { path:'/requisitions',  icon:'bi bi-receipt-cutoff',      label:'Fee Requisitions',    roles:['hod','technician','accountant','daf'] },
  { path:'/leave',         icon:'bi bi-calendar-event-fill', label:'Leave & Permissions', roles:['ceo','hod','team_leader','technician','accountant','daf'] },
  { path:'/salary',        icon:'bi bi-wallet2',             label:'Salary',              roles:['ceo','daf','hod','team_leader','technician','accountant'] },
  { path:'/payments',      icon:'bi bi-credit-card-fill',    label:'Payments',            roles:['ceo','accountant','technician','daf'] },
  { path:'/performance',   icon:'bi bi-graph-up-arrow',      label:'Performance',         roles:['ceo','hod','team_leader','technician','daf'] },
  { path:'/team',          icon:'bi bi-people-fill',         label:'Team',                roles:['ceo','hod','team_leader','daf'] },
  { path:'/sites',         icon:'bi bi-building-fill',       label:'Sites',               roles:['ceo','hod','team_leader','technician'] },
  { path:'/chat',          icon:'bi bi-chat-dots-fill',      label:'Messages',            roles:['ceo','hod','team_leader','technician','accountant','daf'] },
  { path:'/announcements', icon:'bi bi-megaphone-fill',      label:'Announcements',       roles:['ceo','hod','team_leader','technician','accountant','daf'] },
  { path:'/promotions',    icon:'bi bi-robot',               label:'AI Promotions',       roles:['ceo'] },
  { path:'/ai-assistant',  icon:'bi bi-robot',               label:'AI Assistant',        roles:['technician','team_leader','hod','daf','accountant'] },
];

const ROLE_ACCENTS = {
  ceo:'#7C3AED', daf:'#EC4899', hod:'#169BD5',
  team_leader:'#06B6D4', technician:'#10B981', accountant:'#F97316',
};
...
export default function Layout({ children }) {
  ...
  const pendingLeave = ['team_leader','hod','ceo'].includes(currentUser.role)
    ? leaveRequests?.filter(r=>r.status==='pending').length || 0 : 0;
  const pendingReq = ['hod','accountant','daf'].includes(currentUser.role)
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        <header style={{ height:56, background:'var(--bg2)', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 1rem', flexShrink:0, boxShadow:'var(--shadow2)', position:'relative', zIndex:50, gap:8 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
            {['ceo','hod'].includes(currentUser.role) && (
