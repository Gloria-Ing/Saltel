...
function computeBonus(user, salaries, reports, tasks) {
  ...
  // Cap at 10% per policy
  bonusPct = Math.min(10, bonusPct);
...
export default function PromotionsPage() {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)' }}>CEO / CFO Access Only</div>
          <div style={{ color: 'var(--text3)', marginTop: 8 }}>This section is restricted to the CEO and CFO.</div>
        </div>
      </div>
    );
  }
      ...
      if (i >= AI_STEPS.length) {
        ...
        setTimeout(() => {
          const techToLeader = USERS.filter(u => u.role === 'technician')
            .map(u => ({ user: u, result: computeScore(u, 'unit_leader'), targetRole: 'unit_leader' }))
            .sort((a, b) => b.result.score - a.result.score);
          const leaderToHod = USERS.filter(u => u.role === 'unit_leader')
            .map(u => ({ user: u, result: computeScore(u, 'regional_coordinator'), targetRole: 'regional_coordinator' }))
            .sort((a, b) => b.result.score - a.result.score);
  ...
  const handleApplyBonus = () => {
    ...
    const base = BASE_SALARIES[applyModal.user.role] || 320000;
    const maxBonus = Math.round(base * 0.10);
    if (applyModal.bonus.bonusAmount > maxBonus) {
      alert(`Cannot exceed 10% cap. Max bonus for this role: RWF ${maxBonus.toLocaleString()}\nAI recommendation has been adjusted to the cap automatically.`);
      return;
    }
    const result = applyBonus(applyModal.user.id, BONUS_MONTH, applyModal.bonus.bonusAmount, applyModal.bonus.reasons.join('; '));
