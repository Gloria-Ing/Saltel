...
export default function Dashboard() {
  ...
  const myTasks = [...tasks].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).filter(t => {
    if (['technician','staff','store_keeper'].includes(role)) return (t.technician_ids || []).includes(currentUser.id);
    if (role === 'unit_leader') return t.leader_id === currentUser.id;
    if (role === 'regional_coordinator') return t.hod_id === currentUser.id;
    return true;
  });
  ...
  const myReports = [...reports].sort((a, b) => new Date(b.submitted_at) - new Date(a.submitted_at)).filter(r => {
    ...
    if (role === 'unit_leader') { const t = tasks.find(x => x.id === r.task_id); return t?.leader_id === currentUser.id; }
    if (role === 'regional_coordinator') { const t = tasks.find(x => x.id === r.task_id); return t?.hod_id === currentUser.id; }
    return true;
  });
  ...
  // CEO / CFO / CBO / DM overview
  if (['ceo', 'cfo', 'cbo', 'dm'].includes(role)) {
    return (
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <StatCard label="Pending Actions" value={tasks.filter(t => t.workflow_stage === 'ceo_created').length} sub="Tasks awaiting RC" color="var(--amber)" icon={<i className="bi bi-lightning-charge-fill" />} />
        </div>
  ...
  if (role === 'technician' || role === 'staff' || role === 'store_keeper') {
  ...
  // Regional Coordinator / Unit Leader view
  const myPerf = performance.filter(p => {
    if (role === 'regional_coordinator') return users.find(u => u.id === p.user_id)?.dept === currentUser.dept;
    if (role === 'unit_leader') return p.user_id !== currentUser.id && users.find(u => u.id === p.user_id)?.dept === currentUser.dept;
    return false;
  });

  return (
    <div>
      <PageHeader title={<span><i className="bi bi-bar-chart-line-fill me-2" style={{ color: '#169BD5' }} /> {role === 'regional_coordinator' ? 'Unit' : 'Team'} Dashboard</span>} subtitle={`${currentUser.dept} ${role === 'regional_coordinator' ? 'Regional Coordinator' : 'Unit Leader'} · ${new Date().toLocaleDateString('en-RW', { weekday: 'long', day: 'numeric', month: 'long' })}`} />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <StatCard label="Today's Reports" value={todayReports.length} sub={`${todayReports.filter(r => r.status === 'approved').length} approved`} color="var(--cyan)" icon={<i className="bi bi-file-earmark-text-fill" />} onClick={() => navigate('/reports')} />
        <StatCard label={role === 'regional_coordinator' ? 'Pending Requisitions' : 'Pending Leave'} value={role === 'regional_coordinator' ? pendingRequisitions : pendingLeave} color="var(--amber)" icon={role === 'regional_coordinator' ? <i className="bi bi-receipt-cutoff" /> : <i className="bi bi-calendar-event-fill" />} onClick={() => navigate(role === 'regional_coordinator' ? '/requisitions' : '/leave')} />
      </div>
