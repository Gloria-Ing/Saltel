...
export default function TasksPage() {
  ...
  const filtered = [...tasks].sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)).filter(t => {
    ...
    if (role==='team_leader') return t.leader_id===currentUser.id || (t.workflow_stage==='leader_assigned' && t.dept===currentUser.dept);
    if (role==='hod')         return t.hod_id===currentUser.id || t.dept===currentUser.dept;
    return true;
  }).filter(t => {
  ...
  const deptLeaders = (dept) => users.filter(u=>u.role==='team_leader'&&u.dept===dept);
  const deptTechs   = (dept) => users.filter(u=>u.role==='technician'&&u.dept===dept);

  const STAGE_LABELS = {
    all:'All', ceo_created:'Pending Supervisor', hod_created:'Pending HoU',
    leader_assigned:'Pending Techs', supervisor_approval_pending:'Supervisor Approval',
    technicians_assigned:'In Progress', completed:'Completed',
  };

  const pendingApproval = role==='hod' ? filtered.filter(t=>t.workflow_stage==='supervisor_approval_pending') : [];
  ...
  return (
    <div>
      <PageHeader
        ...
        action={['ceo','hod'].includes(role)&&<Btn variant="primary" onClick={()=>navigate('/tasks/new')}>＋ New Task</Btn>}
      />
      ...
      {filtered.length===0 ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {filtered.map(t => {
            ...
            const canHodAssign = role==='hod' && (t.workflow_stage==='ceo_created'||t.workflow_stage==='hod_created') && t.dept===currentUser.dept;
            const canLeaderAssign = role==='team_leader' && t.workflow_stage==='leader_assigned' && t.leader_id===currentUser.id;
            const canComplete = (role==='hod'||role==='team_leader') && t.workflow_stage==='technicians_assigned';
            const canSupervisorApprove = role==='hod' && t.workflow_stage==='supervisor_approval_pending' && t.hod_id===currentUser.id;
            return (
              <Card key={t.id} style={{ borderLeft:`4px solid ${t.priority==='urgent'?'var(--red)':t.priority==='high'?'var(--amber)':'var(--primary)'}` }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                  ...
                  <div style={{ display:'flex', gap:6, flexShrink:0, marginLeft:12, flexWrap:'wrap' }}>
                    ...
                    {canHodAssign   && <Btn size="sm" variant="primary" onClick={()=>openAssign(t,'hod')}>Assign HoU</Btn>}
                ...
                <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
                  {hod&&<div style={{ display:'flex', gap:6, alignItems:'center' }}><Avatar initials={hod.avatar} size={24}/><span style={{ fontSize:12, color:'var(--text3)' }}>Supervisor: {hod.name.split(' ')[0]}</span></div>}
      ...
      {viewTask && (() => {
        ...
        const canHodAssign = role==='hod' && (viewTask.workflow_stage==='ceo_created'||viewTask.workflow_stage==='hod_created') && viewTask.dept===currentUser.dept;
        const canLeaderAssign = role==='team_leader' && viewTask.workflow_stage==='leader_assigned' && viewTask.leader_id===currentUser.id;
        const canDesignateReporter = (role==='team_leader'||role==='hod') && techs.length>1 && (viewTask.leader_id===currentUser.id || viewTask.hod_id===currentUser.id);
        const canSupervisorApprove = role==='hod' && viewTask.workflow_stage==='supervisor_approval_pending' && viewTask.hod_id===currentUser.id;
        return (
          <Modal open={true} onClose={()=>setViewTask(null)} title={viewTask.title} maxWidth={640}>
            ...
            {viewTask.supervisor_rejection_reason && (
              <AlertBanner type="error">⚠ Rejected by supervisor: {viewTask.supervisor_rejection_reason}</AlertBanner>
            )}
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:'1rem' }}>
              {hod&&<div style={{ display:'flex', gap:10, alignItems:'center', padding:'9px 12px', background:'var(--purple-l)', borderRadius:9, fontSize:13 }}><Avatar initials={hod.avatar} color="var(--purple)" size={30}/><div><div style={{ fontWeight:600 }}>{hod.name}</div><div style={{ fontSize:11, color:'var(--text3)' }}>Supervisor</div></div></div>}
              {leader&&<div style={{ display:'flex', gap:10, alignItems:'center', padding:'9px 12px', background:'var(--cyan-l)', borderRadius:9, fontSize:13 }}><Avatar initials={leader.avatar} color="var(--cyan)" size={30}/><div><div style={{ fontWeight:600 }}>{leader.name}</div><div style={{ fontSize:11, color:'var(--cyan)' }}>HoU</div></div></div>}
              {techs.map(u=>(
            ...
            {viewTask.hod_notes&&<div style={{ padding:'10px 14px', background:'var(--bg3)', borderRadius:9, marginBottom:8, fontSize:13 }}>📝 Supervisor: {viewTask.hod_notes}</div>}
            ...
            <div style={{ display:'flex', gap:8, marginTop:'1rem', flexWrap:'wrap' }}>
              {canHodAssign && <Btn variant="primary" onClick={()=>{ setViewTask(null); openAssign(viewTask,'hod'); }}>Assign HoU</Btn>}
      ...
      {assignModal && assignType==='hod' && (
        <Modal open={true} onClose={()=>setAssignModal(null)} title={`Assign HoU — ${assignModal.title}`} maxWidth={480}>
          ...
          <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
            ...
            <Btn variant="primary" onClick={handleHodAssign} disabled={!leaderId}>Assign HoU</Btn>
          </div>
        </Modal>
      )}
